# -*- coding: utf-8 -*-
"""
Created on Wed Oct 21 15:42:35 2020
...
PATTERN_23 = my_defines['PAT_23']
PATTERN_24 = my_defines['PAT_24']
PATTERN_25 = my_defines['PAT_25']
PATTERN_26 = my_defines['PAT_26']
PATTERN_27 = my_defines['PAT_27']
PATTERN_28 = my_defines['PAT_28']
PATTERN_29 = my_defines['PAT_29']
PATTERN_30 = my_defines['PAT_30']
...
def ppg(fw, fh, bit_depth=0, gen_type=0, bw=10, bh=10, p1=0, p2=0):
    ...
    if(gen_type == 19):
        if(1):
            ...
            if(pat_type == 0):
                frame_2 = np.random.choice([0,pixel_max_value],size=(fw,fh))
            if(pat_type == 1):
                frame_2 = np.random.choice([0,1],size=(fw,fh))
            if(pat_type == 2):
                frame_2 = np.random.choice([0,1,2,4,8,16,32,64,128,256,512],size=(fw,fh))
            if(pat_type == 3):
                frame_2 = np.random.choice([0,1,2,3,4,7,8,15,16,31,32,63,64,127,128,255,256,511,512],size=(fw,fh))
            if(pat_type == 4):
                frame_2 = np.random.choice([0,(pixel_max_value+1)/2,(pixel_max_value+1)/2-1,pixel_max_value],size=(fw,fh))
            if(pat_type == 5):
                frame_2 = np.random.choice([0,(pixel_max_value+1)/4,(pixel_max_value+1)/4-1,(pixel_max_value+1)/2,(pixel_max_value+1)/2-1,pixel_max_value],size=(fw,fh))
            if(pat_type == 6):
                pix_start = int(np.random.randint(3,pixel_max_value+1, size=(1,1)))
                frame_2 = np.random.choice([pix_start,pix_start-1,pix_start-2,pix_start-3],size=(fw,fh))
            if(pat_type == 7):
                pix_start = int(np.random.randint(0,pixel_max_value-6, size=(1,1)))
                frame_2 = np.random.choice([pix_start,pix_start+2,pix_start+4,pix_start+6],size=(fw,fh))
    ...
    if(gen_type == 20):
        ...
        for k in range(pqr):
            for l in range(pqr):
                ...
                if(bit_depth == 0):
                    pixel_value = np.random.choice([1,127,128,255,255])
                else:
                    pixel_value = np.random.choice([1,127,128,255,256,511,512,1022,1023])
                    ...
                    for j in range(0,16,1):
                        ...
                        if((pat_type == 0 and j==i+r1) or
                           ...
                           (pat_type == 6 and ((j==i+r1) or (j==i+r1-1)))
                                ...
                                frame_2[16*k+i][16*l+j] = pixel_value;
    ...
    if(gen_type == 22):
        frame_2 = np.random.choice([0,0,0,0,0],size=(fw,fh))
        
    if(gen_type == 23):
        if(bit_depth == 0):
            frame_2 = np.random.choice([255,255],size=(fw,fh))
        else:
            frame_2 = np.random.choice([1023,1023],size=(fw,fh))
...
pattern_type = int(np.random.randint(0,24, size=(1,1)))
#pattern_type = np.random.choice([0,22,21])
pattern_type = 23
fw = 256;
fh = 256;
...
total = len(sys.argv)       
if (total >= 2): 
    bitdepth = str(sys.argv[1]);
    if(bitdepth != 0):
        bitdepth = 1;
else:
    bitdepth = 0;
    #bitdepth = int(np.random.randint(0,1, size=(1,1)))
...
if(pattern_type == PATTERN_22):    
    ppg(fw,fh,bitdepth,PATTERN_22)
if(pattern_type == PATTERN_23):    
    ppg(fw,fh,bitdepth,PATTERN_23)
if(pattern_type == PATTERN_24):    
    ppg(fw,fh,bitdepth,PATTERN_24)
if(pattern_type == PATTERN_25):    
    ppg(fw,fh,bitdepth,PATTERN_25)
if(pattern_type == PATTERN_26):    
    ppg(fw,fh,bitdepth,PATTERN_26)
if(pattern_type == PATTERN_27):    
    ppg(fw,fh,bitdepth,PATTERN_27)
if(pattern_type == PATTERN_28):    
    ppg(fw,fh,bitdepth,PATTERN_28)
if(pattern_type == PATTERN_29):    
    ppg(fw,fh,bitdepth,PATTERN_29)
if(pattern_type == PATTERN_30):    
    ppg(fw,fh,bitdepth,PATTERN_30)
