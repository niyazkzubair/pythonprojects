...
for line in open('FE_REGR_108_1200PM.txt'): #FE_RANDOM_REGR
#for line in open('TEMP.txt'): #FE_RANDOM_REGR
    #print (line)
    ...
    if(('lr_001_' in line or 'lr_010_' in line or 'lr_100_' in line or 'lr_110_' in line or 'lr_111_' in line or 'lr_011_' in line or 'lr_101_' in line) and ('tile' not in line)):
    #if(0):
        if('lr_100_' in line):
            test_type.append('DB')
            type_flag = 1
...
for i,row in test_error_type.iterrows():
    ...
    if('tile' in i):
        ...
        type_flag = 5
    if(('lr_001_' in i or 'lr_010_' in i or 'lr_100_' in i or 'lr_110_' in i or 'lr_111_' in i or 'lr_011_' in i or 'lr_101_' in i)):
    #if(0):
        if('lr_100_' in i):
            if(type_flag != 0):
                test_type.append('_')
...
pd5 =  (pd.pivot_table(test_error_type,index=[("COUNT","TEST_TYPE")],values=[("COUNT","CSIM_CRASH")],aggfunc='sum'))

pd2.to_excel(writer, 'ERROR_CAT_VIEW_4')

status = pd.concat([pd3, pd4, pd5], axis=1)
status.columns = ['TOTAL','PASS','CSIM_CRASH']
status.insert(3,"%PASS",(status["PASS"]/status["TOTAL"])*100,True)
status.loc['TOTAL'] = [status['TOTAL'].sum(),status['PASS'].sum(),status['CSIM_CRASH'].sum(),(status['PASS'].sum()/status['TOTAL'].sum()*100)]
